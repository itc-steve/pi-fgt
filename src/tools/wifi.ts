...
export function registerWifiTools(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    description:
      "rogue APs (monitor/wifi/rogue_ap). Read-only. Bounded — can be large. " +
      "Live (is_dead=false) listed first; use live_only=true to drop dead history.",
    ...
    async execute(_id, params, signal) {
      try {
        ...
        if (Array.isArray(data)) {
          const liveOnly = !!params.live_only;
          if (liveOnly) data = data.filter((r: any) => r && r.is_dead === false);
          // Live first so truncation keeps useful rows (A+B)
          data = [...data].sort((a: any, b: any) => Number(!!a?.is_dead) - Number(!!b?.is_dead));
        }
        data = bounded(
          data,
